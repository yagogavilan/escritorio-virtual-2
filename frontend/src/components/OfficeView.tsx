...
const CreateRoomModal: React.FC<{ onClose: () => void; onCreate: (data: { name: string, color: string, image: string, type: 'fixed', icon: string }) => void; }> = ({ onClose, onCreate }) => {
    const [name, setName] = useState('');
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
            <div className="bg-white rounded-2xl w-full max-w-md p-6 shadow-2xl">
                <div className="flex justify-between items-center mb-4">
                    <h3 className="text-xl font-bold text-slate-800">Nova Sala</h3>
                    <button onClick={onClose}><X size={20} className="text-slate-400" /></button>
                </div>
                <div className="space-y-4">
                    <div>
                        <label className="block text-sm font-semibold text-slate-700 mb-1">Nome da Sala</label>
                        <input
                            value={name}
                            onChange={e => setName(e.target.value)}
                    ...
                    <div>
                        ...
                        >
                            ...
                            <option value="armchair">Poltrona</option>
